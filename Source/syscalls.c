...
ssize_t _read_r(struct _reent *r, int fd, void *ptr, size_t len)
{
    ...
    switch (device) {
    case FD_DEV_RS232:   return term_rs232_ops.read_r(r, fd, ptr, len);
...
ssize_t _write_r(struct _reent *r, int fd, const void *ptr, size_t len)
{
    int device = fd & 0xF000;
    switch (device) {
    case FD_DEV_RS232:   return term_rs232_ops.write_r(r, fd, ptr, len);
...
ssize_t stdin_chars_avail(void)
{
    ...
    int device = fd & 0xF000;
    switch (device) {
    case FD_DEV_RS232:  return term_rs232_ops.chars_avail_r(_impure_ptr, fd);
...
void __assert_func(const char *file, int line, const char *func, const char *failedexpr)
{
    ...
    rs232_poll_send("assertion \"");
    rs232_poll_send(failedexpr);
    rs232_poll_send("\" failed: file \"");
    rs232_poll_send(file);
    rs232_poll_send("\", line ");

    char line_s[16];
    sprintf(line_s, "%d", line);
    rs232_poll_send(line_s);

    if (func) {
        rs232_poll_send(", function ");
        rs232_poll_send(func);
    }
...
void vApplicationStackOverflowHook(TaskHandle_t *pxTask, char *pcTaskName)
{
    ...
    rs232_poll_send("Stack overflow in ");
    rs232_poll_send(pcTaskName);
    rs232_poll_send("\n");
